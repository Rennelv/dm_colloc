...
TEST(test_LED_P_Q, ZeroCoefWithZeroDegree) {
    std::map<LongNatural, LongRational> fst_pol;
    LongRational num1 = LongRational(LongInteger("0"), LongNatural("1"));
    fst_pol.emplace(LongNatural("0"), num1);
    Polynomial a(fst_pol);
    EXPECT_EQ(LED_P_Q(a), num1);  // из многочлена 0x^0 возвращает старший коэффициент 0
}
...
TEST(test_LED_P_Q, ZeroCoef) {
    ...
    fst_pol.emplace(LongNatural("0"), num4);
    Polynomial a(fst_pol);
    EXPECT_EQ(LED_P_Q(a), num1);  // из многочлена 0x^3 + 3x^2 + 6x^1 + 4x^0 возвращает старший коэффициент 0
}
